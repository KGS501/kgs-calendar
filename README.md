<img width="1794" height="876" alt="0" src="https://github.com/user-attachments/assets/b2c5feb6-cbca-4330-bf6d-fe05e6e2ead8" />


Create events and tasks, manage reminders, search your schedule, and switch between agenda, day, 3-day, month, and task views. Use it offline with a local calendar, or connect Nextcloud calendars and task lists when you want everything synced.

**Key features:**

- Events and tasks in one app
- Agenda, day, 3-day, month, and task-list views
- Local calendar support without a server
- Nextcloud calendar and task sync
- Read-only ICS/CalDAV calendar subscriptions
- Reminders for events and tasks
- Recurring events and tasks
- Search across events and tasks
- Calendar colors, ordering, and visibility controls
- Light and dark themes
- English and German app language support
